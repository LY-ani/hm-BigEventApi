...
```
CREATE TABLE `bigevent`.`Untitled`  (
...
1.在 `router/article.js` 下添加路由、验证规则、查询文章处理函数
...
const { addArticle, searchArticle } = require('../router_handle/article');

router.post('/search', expressJoi(search_article_schema), searchArticle);
```

2.在 `schema/article.js` 下添加验证规则并导出
...
3. 在 `router_handle/article.js` 下编写文章查询处理函数并导出
...
1.在 `router/article.js` 下添加路由、验证规则、修改文章处理函数

```js
const { edit_article_schema } = require('../schema/article');

const { editArticle } = require('../router_handle/article');
...
3. 在 `router_handle/article.js` 下编写修改文章处理函数并导出

```js
// 修改文章 应该现在前端通过点击文章修改按钮，然后通过搜索文章id获取文章的内容，再将搜索到的内容展示在前端界面，修改完文章内容后点击确认按钮将文章的内容提交的修改链接，实现修改
// 下面的第一个查询语句就不需要写了，不过我懒得删就不管了
const editArticle = (req, res) => {
  let sql = `select * from ev_articles where id=${req.body.id}`;
  db.query(sql, (err, results) => {
    if (err) return res.cc(err);
    ...
    const sql = `update ev_articles set ? where id=${req.body.id}`;
    db.query(sql, articleInfo, (err, results) => {
      if (err) return res.cc(err);
...
1.在 `router/article.js` 下添加路由、验证规则、删除文章处理函数

```js
const { delete_article_schema } = require('../schema/article');

const { deleteArticle } = require('../router_handle/article');
// 删除文章
...
3. 在 `router_handle/article.js` 下编写删除文章处理函数并导出

```js
// 删除文章
const deleteArticle = (req, res) => {
  const sql = `update ev_articles set is_delete=1 where id=${req.query.id}`;
  db.query(sql, (err, results) => {
    if (err) return res.cc(err);
